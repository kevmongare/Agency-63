    ...
    <footer className="bg-[#0a0a0a] border-t border-white/5">
      <div className="max-w-7xl mx-auto px-6 py-16">
            ...
            <div className="flex gap-3">
              {socials.map(({ label, href }) => (
                <a
                  key={label}
                  href={href}
                  className="w-9 h-9 rounded-lg bg-[#1e1e1e] flex items-center justify-center text-white/40 hover:text-black transition-all duration-200 text-xs font-bold"
                  style={{}}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = 'linear-gradient(135deg, #2DE2C8, #7B5CF6)'; }}
          ...
          {Object.entries(footerLinks).map(([heading, links]) => (
            <div key={heading}>
              ...
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-white/40 text-sm hover:text-[#2DE2C8] transition-colors">{link}</a>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-white/30 text-xs">© {new Date().getFullYear()} Agency 63. All rights reserved.</p>
